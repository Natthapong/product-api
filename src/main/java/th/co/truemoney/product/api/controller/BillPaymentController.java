...
public class BillPaymentController extends BaseController {
	...
	ProductResponse getKeyInBillPayment(
		...
		Map<String, Object> data = BillResponse.builder()
				.setPaymentDraft(paymentDraft)
				.buildBillInfoResponse();
		if(ValidateUtil.isMobileNumber(paymentDraft.getBillInfo().getRef1())){
			String formattedMobileNumber = Utils.formatMobileNumber(paymentDraft.getBillInfo().getRef1());
			data.put("ref1", formattedMobileNumber);
		}else if(ValidateUtil.isTelNumber(paymentDraft.getBillInfo().getRef1())){
			String formattedTelNumber = Utils.formatTelNumber(paymentDraft.getBillInfo().getRef1());
			data.put("ref1", formattedTelNumber);
		}

...
public class TestBillPaymentControllerUnit {
	...
	public void confirmBillPaySkipOTPChecking() throws Exception {
		BillPaymentDraft otpConfirmedDraft = new BillPaymentDraft(null, null,
				null, null, BillPaymentDraft.Status.OTP_CONFIRMED);
		when(
				this.billPaymentServiceMock.getBillPaymentDraftDetail(
						anyString(), anyString()))
				.thenReturn(otpConfirmedDraft);

		when(
				this.billPaymentServiceMock.performPayment(anyString(),
						anyString())).thenReturn(
				BillPaymentTransaction.Status.PROCESSING);

		billPaymentController.confirmBillPayment("", "",
				new HashMap<String, String>());

		verify(this.transactionAuthenServiceMock, never()).verifyOTP(
				anyString(), any(OTP.class), anyString());

		verify(this.billPaymentServiceMock, times(1)).performPayment(
				anyString(), anyString());
	}

	@Test
	public void confirmBillPayWithOTPChecking() throws Exception {
		BillPaymentDraft otpSentDraft = new BillPaymentDraft(null, null, null,
				null, BillPaymentDraft.Status.OTP_SENT);
		when(
				this.billPaymentServiceMock.getBillPaymentDraftDetail(
						anyString(), anyString())).thenReturn(otpSentDraft);

		when(
				this.transactionAuthenServiceMock.verifyOTP(anyString(),
						any(OTP.class), anyString())).thenReturn(
				DraftTransaction.Status.OTP_CONFIRMED);
		...
		verify(this.transactionAuthenServiceMock, times(1)).verifyOTP(
				anyString(), any(OTP.class), anyString());
	...
	public void createBillPaymentSuccess() {
		...
		when(
				billPaymentServiceMock.verifyPaymentAbility(anyString(),
						any(BigDecimal.class), anyString())).thenReturn(bill);

		when(transactionAuthenServiceMock.requestOTP(anyString(), anyString()))
				.thenReturn(new OTP());

		ProductResponse resp = billPaymentController.createBillPayment(
				fakeAccessTokenID, request);
		Map<String, Object> data = resp.getData();
		assertNotNull(data);
	...
	public void confirmBillPaymentSuccess() throws Exception {
		...
		ProductResponse resp = billPaymentController.confirmBillPayment(
				"111111", fakeAccessTokenID, request);
		Map<String, Object> data = resp.getData();
		assertNotNull(data);
	...
	public void getBillPaymentStatusSuccess() throws Exception {
		when(
				billPaymentServiceMock.getBillPaymentStatus(anyString(),
						anyString())).thenReturn(
				BillPaymentTransaction.Status.PROCESSING);

		ProductResponse resp = billPaymentController.getBillPaymentStatus(
				"11111", fakeAccessTokenID);
		Map<String, Object> data = resp.getData();
		assertNotNull(data);
	...
	public void getBillPaymentDetailSuccess() throws Exception {
		...
		when(
				billPaymentServiceMock.getBillPaymentResult(anyString(),
						anyString())).thenReturn(bpay);

		when(profileServiceMock.getEwalletBalance(anyString())).thenReturn(
				new BigDecimal(100.00));

		ProductResponse resp = billPaymentController.getBillPaymentDetail(
				"111111", fakeAccessTokenID);
		Map<String, Object> data = resp.getData();
		assertNotNull(data);
	...
	@Test
	public void getInquiryBillInformation() throws ParseException {
		Map<String, String> req = new HashMap<String, String>();
		req.put("ref1", "0891234567");
		...
		ProductResponse resp = billPaymentController.getBillOnlineInformation(
				fakeAccessTokenID, req);
...
//		assertEquals("Convergence Postpay", data.get("titleEn"));		
	...
	private BillPaymentDraft createBillPaymentDraftStubbed() throws Exception {
		Bill bill = createStubbedBillInfo();
		return new BillPaymentDraft("1111111111", bill, new BigDecimal(11000),
				"123567890", Status.OTP_CONFIRMED);
	}
